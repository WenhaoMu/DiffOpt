...
def run_evaluate(
    ...
    def _get_classifier():
        checkpoint_path = f"./classifier/gp_model.pth"
        # checkpoint_path = f"experiments/{taskname}/branin_1_1/123/wandb/latest-run/files/checkpoints/last.ckpt"
        loaded_gp = SingleTaskGP(train_X, train_Y)

        # 现在，加载模型参数
        loaded_gp.load_state_dict(torch.load("gp_model.pth"))
        loaded_gp.eval()  # 设置为评估模式
        return loaded_gp
    ...
    def cond_fn(x, t, y=None):
        ...
        with torch.enable_grad():
            ...
            # 计算均值和方差的梯度
            mean_grad, = torch.autograd.grad(mean.mean(), x_in, retain_graph=True)  # 注意这里的逗号，因为grad返回一个元组
    ...
    filename = f"./final_solution/{args.task}_mlptest512.csv"
