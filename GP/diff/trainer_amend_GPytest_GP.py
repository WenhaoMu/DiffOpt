...
def run_evaluate(
    ...
    # set_seed(seed)
    # taskGP = design_bench.make(TASKNAME2TASK[taskname])
    # datasetGP = taskGP.dataset
    # datasetGP.subsample(max_samples=200)
    # print("Y1:", taskGP.y)
    ...
    # train_X = torch.from_numpy(taskGP.x)
    # train_Y = torch.from_numpy(taskGP.y)
    ...
    def _get_classifier():
        # checkpoint_path = f"./classifier/gp_super50.pth"
        # checkpoint_path = f"experiments/{taskname}/branin_1_1/123/wandb/latest-run/files/checkpoints/last.ckpt"
        # loaded_gp = SingleTaskGP(train_X, train_Y)
        num_inducing = 40
        m_GP = GPy.models.SparseGPRegression(X=train_X, Y=train_Y, kernel=GPy.kern.RBF(train_X.shape[1])+GPy.kern.Bias(train_X.shape[1]), num_inducing=num_inducing)

        # loaded_gp.load_state_dict(torch.load("./classifier/gp_super500.pth"))
        # loaded_gp.eval()  
        m_GP.update_model(False) # do not call the underlying expensive algebra on load
        m_GP.initialize_parameter() # Initialize the parameters (connect the parameters up)
        m_GP.param_array[:] = np.load('./classifier_GPy/model_save_GP.npy') # Load the parameters
        # m_.param_array[:] = np.load('./classifier_GPy/model_save.npy')

        m_GP.update_model(True) # Call the algebra only once
        # m_GP.eval() 
        return m_GP
    ...
    def cond_fn(x, t, y=None):
        ...
        with torch.enable_grad():
            ...
            print("X_in: ",type(x_in), x_in.shape)
            # y_pred = classifier.mlp(x_in, t)  # y_pred represents the continuous predicted values
            # posterior = classifier.posterior(x_in)
            mean, variance = classifier.predict(x_in)
            # mean = posterior.mean
            # variance = posterior.variance

            print("mean: ",type(mean), mean.shape)
            print("variance: ",type(variance), variance.shape)

        
            mean_grad, = torch.autograd.grad(mean.mean(), x_in, retain_graph=True)  
            variance_grad, = torch.autograd.grad(variance.mean(), x_in)

            # print(x_in.shape, y_pred.shape)
            # print(x_in)
            # print(y_pred)
            # loss = (y_pred.view(-1) - y.view(-1))**2  # compute MSE loss
            print("Mean grad: ", type(mean_grad), mean_grad.shape)
            print("Var grad: ", type(variance_grad), variance_grad.shape)
            print(mean_grad)
            print(variance_grad)

            grad_y_pred = mean_grad + variance_grad
    ...
    if not os.path.exists('./final_solution'):
        os.makedirs('./final_solution')

    filename = f"./final_solution/{args.task}_GP500.csv"
