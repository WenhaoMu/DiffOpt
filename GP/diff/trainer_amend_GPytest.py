    ...
    def _get_classifier():
        ...
        m = deepgp.DeepGP([train_Y.shape[1],hidden,train_X.shape[1]],Y=train_Y, X=train_X, inits=['PCA','PCA'], 
                        kernels=[GPy.kern.RBF(hidden,ARD=True),
                                GPy.kern.RBF(train_X.shape[1],ARD=True)+ GPy.kern.Bias(train_X.shape[1])], # the kernels for each layer
        ...
        # m[:] = np.load('./classifier_GPy/model_save.npy') # Load the parameters
        m.param_array[:] = np.load('./classifier_GPy/model_save.npy')
