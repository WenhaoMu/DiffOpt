...
def run_evaluate(
    ...
    def _get_classifier():
        ...
        classifier.load_state_dict(torch.load('/localscratch/wmu30/GP/DGP_model/best_model_{}.pth'.format(taskname)))
        return classifier
    ...
    def cond_fn(x, t, y=None):
        ...
        with torch.enable_grad():
            ...
            # print("XIN: ",x_in)
            # predictive_means, predictive_variances, test_lls = classifier.predict(x_in)
            # predictive_means, predictive_variances = classifier.predict(x_in)
            output = classifier(x_in)
            predictive_means = output.mean.mean(0)
            predictive_variances = output.variance.mean(0)
            # print("MEAN: ",predictive_means)
            # print("VAR: ",predictive_variances)
            # print("mean: ",type(predictive_means), predictive_means.shape)
            # print("variance: ",type(predictive_variances), predictive_variances.shape)
            ...
            # print("VAR GRAD: ", variance_grad)

            # grad_y_pred = mean_grad + variance_grad
            # grad_y_pred = variance_grad * 10
            grad_y_pred = mean_grad
            # print("FINAL GRAD: ", grad_y_pred)
    ...
    def heun_sampler(sde, x_0, ya, num_steps, lmbd=0., keep_all_samples=True):
        ...
        with torch.no_grad():
            for i in range(num_steps):
                ...
                # 计算 gradient 在 mu 上的投影
                proj_on_mu = torch.sum(gradient * mu, dim=-1, keepdim=True) / torch.sum(mu * mu, dim=-1, keepdim=True) * mu

                # 计算垂直于 mu 的分量
                perpendicular_to_mu = gradient - proj_on_mu

                # 更新 mu
                mu = mu.float() + args.coefficient * perpendicular_to_mu.float()
    ...
    if not os.path.exists('./final_solution_mlpproQ'):
        os.makedirs('./final_solution_mlpproQ')

    filename = f"./final_solution_mlpproQ/{args.task}_mlppro_Qtest.csv"
