...
def run_evaluate(
    ...
    for i in range(10, 1001, 10):
        ...
        # save to file
        expt_save_path = f"./experiments/{args.task}/{args.name}/{args.seed}_7"
        ...
        for lmbd in lmbds:
            ...
            print("data max: {}".format(args.condition))
            ...
            for qqq in xs:
                ...
                if not qqq.isnan().any():
                    ...
                    if normalise_y:
                        ...
                        print("result:{}".format(ys.max()))
                        y_max = task.denormalize_y(task.y.max())
                        value_max.append(ys.max().item())
                        # y_mean = task.denormalize_y(task.y.mean())
                        value_mean.append(ys.mean().item())
                        print("data max: {}".format(y_max))
                    else:
                        print("none")
                        print(ys.max())
                    results.append(ys)
    ...
    # 显示图表
    plt.savefig(f"diffusion_plot/superconductor_{args.seed}_7.png")
