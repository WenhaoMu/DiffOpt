...
def run_classifier(
    ...
    model.load_state_dict(torch.load('/localscratch/wmu30/GP/DGP_model/best_model_{}_old.pth'.format(taskname)))
    ...
    for epoch in range(num_epochs):
        ...
        val_loss = 0.0
        num_val = 0
        for val_X, val_Y,_ in train_loader:
            val_X = val_X.cuda()
            val_Y = val_Y.cuda()
            with torch.no_grad():
                output = model(val_X)
                loss = -mll(output, val_Y.shape[0])
                predictive_means, predictive_variances = model.predict(val_X)
                print("VAL X")
                print(val_X)
                print("MEAN")
        ...
        if val_loss < best_val_loss:
            ...
            # torch.save(model.state_dict(), os.path.join("DGP_model", f"best_model_{taskname}.pth"))
