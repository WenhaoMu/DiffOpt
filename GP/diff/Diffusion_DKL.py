...
def run_evaluate(
    ...
    def _get_classifier():
        ...
        classifier = GPRegressionModel(dim_x)
        classifier.load_state_dict(torch.load('/localscratch/wmu30/GP/DKL_model/best_model_{}_test.pth'.format(taskname)))
        return classifier
    ...
    def cond_fn(x, t, y=None):
        ...
        with torch.enable_grad():
            ...
            # print("MEAN: ",predictive_means.shape)
            # print("VAR: ",predictive_variances.shape)
            # print("mean: ",type(predictive_means), predictive_means.mean(0).shape)
            # print("variance: ",type(predictive_variances), predictive_variances.mean(0).shape)
            ...
            # print("MEAN: ", type(predictive_means.mean()), predictive_means.mean().shape)
            ...
            # grad_y_pred = 2 * variance_grad + mean_grad
    ...
    if not os.path.exists('./DKL_solution'):
        os.makedirs('./DKL_solution')

    filename = f"./DKL_solution/{args.task}_DKL.csv"
