...
#   python design_baselines/diff_branin/trainer.py --config $CONFIG --seed $seed --use_gpu --mode 'eval' --task $TASK 
  ...
  # python design_baselines/diff/classifier.py --config $CONFIG --seed $seed --use_gpu --mode 'train' --task $TASK
  python design_baselines/diff/classifier_fourier.py --config $CONFIG --seed $seed --use_gpu --mode 'train' --task $TASK --which_gpu 5
done
