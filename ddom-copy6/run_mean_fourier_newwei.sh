...
for seed in $seeds; do
  for Coefficient in $Coefficients; do
    ...
    python design_baselines/diff/generation_mean_fourier_newwei.py --config $CONFIG --seed $seed --use_gpu --mode 'eval' --task $TASK --coefficient $Coefficient --which_gpu 7 --suffix "max_ds_conditioning"
  done
done
