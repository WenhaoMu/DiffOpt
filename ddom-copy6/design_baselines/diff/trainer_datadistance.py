...
from datadistance_model import Classifier as Datadistance
...
def run_evaluate(
    ...
    def _get_distance():
        checkpoint_path = f"experiments/{taskname}/datadistance/123_nonorm/wandb/run-20230904_133743-ccgbjqxt/files/checkpoints/last.ckpt"
        # checkpoint_path = f"experiments/{taskname}/branin_1_1/123/wandb/latest-run/files/checkpoints/last.ckpt"
        datadistance = Datadistance.load_from_checkpoint(
            checkpoint_path=checkpoint_path,
            taskname=taskname,
            task=task,)
        return datadistance
    ...
    def heun_sampler(sde, x_0, ya, num_steps, lmbd=0., keep_all_samples=True):
        ...
        with torch.no_grad():
            for i in range(num_steps):
                ...
                uncertainty = datadistance(x_t)
                print("gradient size: ",gradient.shape)
                print("mu size: ",mu.shape)
                print("uncertainty size: ",uncertainty.shape)
                # print("input size: ",x_t.shape)
                mu = (
                    # mu.float() + sigma * gradient.float()
                    mu.float() + args.coefficient *gradient.float() + uncertainty
                )
    ...
    filename = f"./solution/{args.task}_amend_uncertainty.csv"
