...
class Compound:
    ...
    def __init__(self):
        with open("./design_baselines/diff_multi/data/chembl_encoded_valid.p", "rb") as f:
            data = pickle.load(f)
        with open("./design_baselines/diff_multi/data/chembl_value_valid.p", "rb") as f:
            value = pickle.load(f)
        self.x = data
        self.y = value[:, [1, 2, 5]]
    ...
    for lmbd in lmbds:
        ...
        for sol in xs:
            ...
            if not sol.isnan().any():
                ...
                os.makedirs('./design_baselines/diff_multi/molecular_discovery_problem/Result_Final_Diff_Gui_Coefficient', exist_ok=True)
                torch.save(sol, f'./design_baselines/diff_multi/molecular_discovery_problem/Result_Final_Diff_Gui_Coefficient/molecular_gui_{args.coefficient}_{num_samples}.pt')
            else:
                print("No solution!")
