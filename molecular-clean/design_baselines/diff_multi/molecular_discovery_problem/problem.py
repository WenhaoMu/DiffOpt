...
def get_problem(name, *args, **kwargs):
    ...
    PROBLEM = {
        ...
        'amortized_f5': F5,
        'amortized_f6': F6,
        'amortized_vlmop2': VLMOP2,
        'amortized_vlmop3': VLMOP3,
        'amortized_dtlz2': DTLZ2,
        'amortized_re21': RE21,
        'amortized_re23': RE23,
        'amortized_re33': RE33,
        'amortized_re36': RE36,
...
class MolecularDiscovery():
    ...
    def __init__(self):
        ...
        self.c_dim = 64
        self.n_dim = 32 + 64
        self.lbound = torch.tensor([0] * 10).float()
        self.ubound = torch.tensor([1] * 10).float()
        self.model = self.SurrogateModel(32, 64).cuda()
        self.model.load_state_dict(torch.load('/localscratch/wmu30/ddom-multi-gui/design_baselines/diff_multi/molecular_discovery_problem/surrogate_model.pt'))
        # self.model.load_state_dict(torch.load('./surrogate_model.pt'))
        self.model.eval()
        self.targets = torch.tensor(np.load('/localscratch/wmu30/ddom-multi-gui/design_baselines/diff_multi/molecular_discovery_problem/target_embeddings.npy')).float().cuda()
        # self.targets = torch.tensor(np.load('./target_embeddings.npy')).float().cuda()
