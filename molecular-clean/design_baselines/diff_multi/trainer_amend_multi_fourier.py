...
from classifier_model_fourier import Classifier
...
def run_evaluate(
    ...
    def _get_classifier():
        ...
        classifier1 = Classifier.load_from_checkpoint(checkpoint_path=checkpoint_path1,taskname=taskname,task=task,)
        checkpoint_path2 = f"experiments/{taskname}/fourier_2/1469983670_128/wandb/latest-run/files/checkpoints/last.ckpt"
        classifier2 = Classifier.load_from_checkpoint(checkpoint_path=checkpoint_path2,taskname=taskname,task=task,)
        checkpoint_path3 = f"experiments/{taskname}/fourier_3/1469983670_128/wandb/latest-run/files/checkpoints/last.ckpt"
        classifier3 = Classifier.load_from_checkpoint(checkpoint_path=checkpoint_path3,taskname=taskname,task=task,)
    ...
    for lmbd in lmbds:
        ...
        for sol in xs:
            ...
            if not sol.isnan().any():
                ...
                torch.save(sol, f'./design_baselines/diff_multi/molecular_discovery_problem/Result_Fourier/molecular_gui_{args.coefficient}_{num_samples}.pt')
