...
class ScorePluginReverseSDE(torch.nn.Module):
    ...
    # Drift
    def mu(self, t, y, lmbd=0., gamma=0.):
    ...
    def dsm(self, x):
        ...
        a = self.a(x_hat, t_.squeeze())

        return ((a * std + target) ** 2).view(x.size(0), -1).sum(1, keepdim=False) / 2

    @torch.enable_grad()
    def dsm_weighted(self, x, w, clip=False, c_min=None, c_max=None):
        """
        denoising score matching loss
        """
        if self.debias:
    ...
    def elbo_random_t_slice(self, x, y_n):
        ...
        a = self.base_sde.g(t_, y) * self.a(y, t_.squeeze())
