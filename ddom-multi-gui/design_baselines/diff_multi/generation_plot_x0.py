...
def run_evaluate(
    ...
    def _get_classifier():
        checkpoint_path = f"experiments/{taskname}/new_classifier_x0/123/wandb/latest-run/files/checkpoints/last.ckpt"
    ...
    def heun_sampler(sde, x_0, ya, num_steps, lmbd=0., keep_all_samples=True):
        ...
        with torch.no_grad():
            for i in range(num_steps):
                ...
                # print("coefficient: {}".format(type(args.coefficient)))
    ...
    os.makedirs("./diffusion_process_plot/x0", exist_ok=True)

    # 显示图表
    # plt.savefig(f"diffusion_plot/{args.task}_{args.seed}_{args.coefficient}_noreweight.png")
    plt.savefig(f"diffusion_process_plot/x0/{args.task}_{args.seed}_{args.coefficient}.png")
