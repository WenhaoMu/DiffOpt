...
class MolecularDiscovery():
    ...
    def __init__(self):
        ...
        # self.model.load_state_dict(torch.load('./design_baselines/diff_multi/molecular_discovery_problem/surrogate_model.pt'))
        self.model.load_state_dict(torch.load('./surrogate_model.pt'))
        self.model.eval()
        # self.targets = torch.tensor(np.load('./design_baselines/diff_multi/molecular_discovery_problem/target_embeddings.npy')).float().cuda()
