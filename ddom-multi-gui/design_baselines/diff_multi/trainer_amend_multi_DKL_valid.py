...
def run_evaluate(
    ...
    def _get_classifier():
        # classifier = DeepGp(dim_x)
        classifier1 = GPRegressionModel(dim_x+1)
        classifier1.load_state_dict(torch.load('/localscratch/wmu30/ddom-multi-gui/DKL_model/best_model_{}_x0_1.pth'.format(taskname)))
        classifier2 = GPRegressionModel(dim_x+1)
        classifier2.load_state_dict(torch.load('/localscratch/wmu30/ddom-multi-gui/DKL_model/best_model_{}_x0_2.pth'.format(taskname)))
        classifier3 = GPRegressionModel(dim_x+1)
        classifier3.load_state_dict(torch.load('/localscratch/wmu30/ddom-multi-gui/DKL_model/best_model_{}_x0_3.pth'.format(taskname)))
    ...
    for lmbd in lmbds:
        ...
        for qqq in xs:
            ...
            if not qqq.isnan().any():
                ...
                print("logp: {}".format(Result[3]))
                print("QED: {}".format(Result[4]))
                print("SA: {}".format(Result[-1]))
    ...
    filename = f"./solution/{args.task}_amend_multi_5.csv"
    if not os.path.exists(filename):
        # 如果不存在，创建文件并写入标题行
        with open(filename, 'w', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow([coe, '', Result[3], '', Result[4], '', Result[-1]])
