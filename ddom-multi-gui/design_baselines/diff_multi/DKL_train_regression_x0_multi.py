...
def run_classifier(
    ...
    # mll = DeepApproximateMLL(VariationalELBO(likelihood, model.gp_layer, train_X.shape[-2]))
    ...
    for epoch in range(num_epochs):
        ...
        for train_X, train_Y,_ in train_loader:
            ...
            # print("X: ", x_hat.shape, "t: ", t_.shape)
            output = model(torch.cat([x_hat, t_],dim=1))
            # print("Y: ", train_Y.shape)
            # print("OUTPUT SHAPE: ", output.mean.shape, "Y shape: ", train_Y.shape)
            # print("OUTPUT: ", output.mean)
            loss = -mll(output, train_Y)
            # loss = -mll(output, train_Y)
            # print("TRAIN LOSS:", loss.shape)
            loss.backward()
            optimizer.step()
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join("DKL_model", f"best_model_{taskname}_x0_1.pth"))
