...
class Compound:
    ...
    def __init__(self):
        # data = torch.load("./design_baselines/diff_multi/data/compounds_data.pt")
        # value = torch.load("./design_baselines/diff_multi/data/compounds_value.pt")
        with open("./design_baselines/diff_multi/data/compound_data.p", "rb") as f:
            data = pickle.load(f)
        with open("./design_baselines/diff_multi/data/compound_value.p", "rb") as f:
            value = pickle.load(f)
...
def run_evaluate(
    ...
    def cond_fn(x, t, y=None):
        ...
        with torch.enable_grad():
            ...
            # print("SIZE CHECK: ", x_in.shape, c.shape)
            # c_batch = c.repeat(512,1)
            c_batch = c.repeat(256,1)
            x_in = torch.cat((x_in, c_batch), dim=1).requires_grad_(True)    
            y_pred = problem.evaluate(x_in, 0)
            y_to_consider_1 = y_pred[:,0]
            y_to_consider_2 = y_pred[:,1]
            y_to_consider_3 = y_pred[:,2]
            # print(y_to_consider)
            grads_1 = torch.autograd.grad(y_to_consider_1.sum(), x_in, retain_graph=True)[0]
            grads_2 = torch.autograd.grad(y_to_consider_2.sum(), x_in, retain_graph=True)[0]
            grads_3 = torch.autograd.grad(y_to_consider_3.sum(), x_in, retain_graph=True)[0]
            # print("GRAD: ",grads.shape)

            grad_input1 = grads_1[:,:32]
            grad_input2 = grads_2[:,:32]
            grad_input3 = grads_3[:,:32]
        ...
        with torch.no_grad():
            for i in range(num_steps):
                ...
                mu = (
                    ...
                    mu.float() + args.coefficient *5*gradient1.float() + args.coefficient *gradient2.float() - args.coefficient *gradient3.float()
                )
    ...
    for i in range(1, 1002, 25):
            ...
            for qqq in xs:
                ...
                if not qqq.isnan().any():
                    ...
                    if not task.is_discrete:
                        ...
                        # ys = problem.evaluate(qqq.cpu(), 0)
                        c = problem.targets[torch.randperm(len(problem.targets))[:1]]  
                        # print("SIZE CHECK: ", x_in.shape, c.shape)
                        # c_batch = c.repeat(512,1)
                        c_batch = c.repeat(256,1)
                        # print("Device of qqq:", qqq.device)
                        # print("Device of c_batch:", c_batch.device)
                    
                        qqq = qqq.to(c_batch.device)

                        x_in = torch.cat((qqq, c_batch), dim=1).requires_grad_(True)  
                        ys = problem.evaluate(x_in, 0)
                    ...
                    # max_index = ys.argmax() 
                    # 计算 ys 的第一列 + 第二列 - 第三列 的值
                    computed_values = ys[:, 0] + ys[:, 1] - ys[:, 2]

                    # 找到这个新向量的最大值的索引
                    max_index = computed_values.argmax()
                    ...
                    print("total energy: {}".format(Result[3]))
                    print("torsional energy: {}".format(Result[4]))
                    print("ligand efficiency: {}".format(Result[9]))
                    # print("RESULT MAX 1: ",Result[0].item())
                    value_max_1.append(Result[3].item())
                    value_max_2.append(Result[4].item())
                    value_max_3.append(Result[-1].item())
                    value_mean_1.append(ys[:,3].mean().item())
                    value_mean_2.append(ys[:,4].mean().item())
                    value_mean_3.append(ys[:,-1].mean().item())
        ...
        # print("--------------------------results----------------------------")
        # print("nor: {}".format(Result))
        # # print("denor: {}".format(denorm_result))
        ...
        # filename = f"./solution/{args.task}_amend_multi_3.csv"
        # if not os.path.exists(filename):
        #     # 如果不存在，创建文件并写入标题行
        #     with open(filename, 'w', newline='') as csvfile:
        #         csvwriter = csv.writer(csvfile)
        #         csvwriter.writerow([coe, '', Result[0], '', Result[1], '', Result[2]])
                
        # # 现在，将数据追加到文件
        # with open(filename, 'a', newline='') as csvfile:
        #     csvwriter = csv.writer(csvfile)
        #     csvwriter.writerow([coe, '', Result[0], '', Result[1], '', Result[2]])
    print("step: {}".format(len(step)))
    print("max: {}".format(len(value_max_1)))
    print("mean: {}".format(len(value_mean_1)))


    # 使用plot函数绘制第一条折线，它使用x作为x坐标，y1作为y坐标
    # plt.plot(step, value_max_1, label='total energe', color='lightblue')
    # plt.plot(step, value_max_2, label='tortional energy', color='dodgerblue')
    # plt.plot(step, value_max_3, label='ligand efficiency', color='navy')

    # 使用plot函数绘制第二条折线，它使用x作为x坐标，y2作为y坐标
    plt.plot(step, value_mean_1, label='mean total energe', color='salmon')
    plt.plot(step, value_mean_2, label='mean tortional energy', color='firebrick')
    plt.plot(step, value_mean_3, label='mean ligand efficiency', color='black')
    ...
    os.makedirs("./diffusion_process_plot/multi", exist_ok=True)

    # 显示图表
    # plt.savefig(f"diffusion_plot/{args.task}_{args.seed}_{args.coefficient}_noreweight.png")
    plt.savefig(f"diffusion_process_plot/multi/{args.task}_{args.seed}_{args.coefficient}_5_new_valid_mean.png")

    os.makedirs("./solution", exist_ok=True)
    with open(f"solution/output_{args.seed}_new.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["value_max_1", "value_max_2", "value_max_3", "value_mean_1", "value_mean_2", "value_mean_3"])
        
        for row in zip(value_max_1, value_max_2, value_max_3, value_mean_1, value_mean_2, value_mean_3):
            writer.writerow(row)
