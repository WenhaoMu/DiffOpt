...
def run_evaluate(
        ...
        for lmbd in lmbds:
            ...
            for qqq in xs:
                ...
                if not qqq.isnan().any():
                    ...
                    print("logp: {}".format(Result[0]))
                    print("QED: {}".format(Result[1]))
                    print("SA: {}".format(Result[2]))
                    # print("RESULT MAX 1: ",Result[0].item())
                    value_max_1.append(Result[0].item())
                    value_max_2.append(Result[1].item())
                    value_max_3.append(Result[2].item())
                    value_mean_1.append(ys[:,0].mean().item())
                    value_mean_2.append(ys[:,1].mean().item())
                    value_mean_3.append(ys[:,2].mean().item())
    ...
    plt.plot(step, value_max_1, label='max lopP', color='lightblue')
    plt.plot(step, value_max_2, label='max QED', color='dodgerblue')
    plt.plot(step, value_max_3, label='min SA', color='navy')

    # 使用plot函数绘制第二条折线，它使用x作为x坐标，y2作为y坐标
    plt.plot(step, value_mean_1, label='mean logP', color='salmon')
    plt.plot(step, value_mean_2, label='mean QED', color='firebrick')
    plt.plot(step, value_mean_3, label='mean SA', color='darkred')
    ...
    plt.savefig(f"diffusion_process_plot/multi/{args.task}_{args.seed}_{args.coefficient}_5_new.png")
