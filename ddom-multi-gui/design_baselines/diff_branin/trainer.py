...
class Branin:
    ...
    def __init__(self, path="design_baselines/diff_branin/dataset/branin_unif_5000.p"):
...
def run_training_forward(
    ...
    checkpoint_dirpath = os.path.join(wandb_logger.experiment.dir,
                                      checkpoint_dir)
    # checkpoint_dirpath = f"experiments/{taskname}/forward_model/123/wandb/latest-run/files/checkpoints"
...
def run_evaluate(
    ...
    def _get_classifier():
        checkpoint_path = f"experiments/{taskname}/new__classifier/123/wandb/latest-run/files/checkpoints/last.ckpt"
    ...
    def euler_maruyama_sampler(sde,
        ...
        with torch.no_grad():
            for i in range(num_steps):
                t.fill_(ts[i].item())
                t2.fill_(ts[i].item())
                mu = sde.gen_sde.mu(t, x_t, lmbd=lmbd, gamma=args.gamma) # ya here should be removed
                g = sde.gen_sde.base_sde.g(1-t2, y_plot)
                # a = sde.gen_sde.a(x_plot, 1-t2.squeeze(), y_plot)
                a = sde.gen_sde.a(x_plot, 1-t2.squeeze())
    ...
    #     # checkpoint_path = f"experiments/{taskname}/score_diffusion/123/wandb/latest-run/files/checkpoints/last.ckpt"
    ...
    for lmbd in lmbds:
        ...
        for qqq in xs:
            ...
            if not qqq.isnan().any():
                ...
                designs.append(np.copy(qqq.cpu().numpy()))
                if not task.is_discrete:
                ...
                max_index = ys.argmax() 
                print("Corresponding qqq for GT ys max: {}".format(qqq[max_index]))
    ...
    os.makedirs('output_plot/newplot', exist_ok=True)
    plt.figure()  # 新建一个图像

    # 先找出在指定范围内的点
    # mask = (designs[:, 0] >= -50) & (designs[:, 0] <= 50) & (designs[:, 1] >= -50) & (designs[:, 1] <= 50)
    # filtered_designs = designs[mask]
    # filtered_results = results[mask]

    # 一次性画出所有的点，这样可以保证颜色条的范围是整个results数组的范围
    # sc = plt.scatter(filtered_designs[:, 0], filtered_designs[:, 1], c=filtered_results)
    sc = plt.scatter(designs[:, 0], designs[:, 1], c=results)

    # plt.xlim(-10, 20)  # 设置x轴的范围
    # plt.ylim(-10, 20)  # 设置y轴的范围
    plt.colorbar(sc)  # 添加颜色条
    plt.savefig(f'output_plot/newplot/{taskname}_{seed}_{num_steps}.png')  # 保存图像到文件，使用样本索引作为文件名的一部分
    plt.clf()  # 清除当前图像，开始画下一个

    # os.makedirs('newplot_seaborn', exist_ok=True)
    # plt.figure()
    ...
    # # 使用sns.kdeplot函数来生成核密度估计图
    # sns.kdeplot(x=filtered_designs[:, 0], y=filtered_designs[:, 1], fill=True)

    # plt.xlim(-8, 12.5)  # 设置x轴的范围
    # plt.ylim(-5, 20)  # 设置y轴的范围

    # plt.xlabel('x1')
    # plt.ylabel('x2')

    # plt.savefig(f'newplot_seaborn/{taskname}_{seed}_{num_steps}.png')  # 保存图像到文件
    # plt.clf()
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        # checkpoint_path = os.path.join(
        #     expt_save_path, "wandb/run-20230726_175757-oem8aden/files/checkpoints/last.ckpt")
