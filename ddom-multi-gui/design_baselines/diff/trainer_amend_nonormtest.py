...
def run_evaluate(
    ...
    def _get_classifier():
        checkpoint_path = f"experiments/{taskname}/new_classifier_fourier/123_nonorm/wandb/latest-run/files/checkpoints/last.ckpt"
    ...
    expt_save_path = f"./experiments/{args.task}/{args.name}/{args.seed}_nonorm"
    ...
    filename = f"./solution/{args.task}_amend_nonorm.csv"
    if not os.path.exists(filename):
        # 如果不存在，创建文件并写入标题行
        with open(filename, 'w', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow([coe, '', norm_result])
